...
const ReferencesPopup = ({ references, onClose }) => {
  ...
  // Nettoyage et traitement des références
      ...
      >
        ...
        <div className="space-y-4">
          {refArray.map((refNumber, index) => {
            ...
            console.log(`Looking for reference ${refNumber}:`, fullReference); // Pour debug
...
const formatColumnName = (column) => {
  const names = {
    ...
    category: 'Catégorie',
    halfLife: 'Demi-vie',
    normofractionatedRT: 'RT Normofractionné',
    palliativeRT: 'RT Palliatif',
    stereotacticRT: 'RT Stéréotaxique',
    intracranialRT: 'RT Intracrânien'
  };
...
const ColumnManager = () => (
  ...
  >
    <div className="flex justify-between items-center mb-3">
      <h3 className="font-semibold text-sfro-dark">Colonnes visibles</h3>
      <button 
...
// Drawer pour filtres mobiles
const MobileFilterDrawer = () => (
  <motion.div 
    initial={{ x: '100%' }}
    animate={{ x: 0 }}
    exit={{ x: '100%' }}
    className="fixed inset-y-0 right-0 w-80 bg-white shadow-lg p-4 z-50"
  >
    <div className="flex justify-between items-center mb-4">
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <option value="endocrine">Thérapie endocrine</option>
        <option value="targeted">Thérapie ciblée</option>
  ...
  {filterAndSortDrugs().map((drug, index) => (
    ...
    {zoomedCell && (
      ...
      >
        <div className="bg-white p-6 rounded-lg max-w-lg m-4">
          <p className="text-lg">{zoomedCell}</p>
        </div>
      </div>
    )}
  </motion.div>
